...
func setup(dir:Vector2,spd:float,dmg:float,shooter,parry:bool,color:Color):
	direction = dir
	speed = spd
	damage= dmg
	...
	if counter_projectile: 
		...
		if body.is_in_group("Player"):
			...
			if not PlayerMovementStats.is_dash and not counter_projectile:
				body.take_damage(damage,current_shooter,GlobalParameters.HIT,0.012)
				queue_free()
